...
bool FCNSourcePin::DoSenderConnection()
{
	...
	if(isMulticast)
	{
		//input = new FCNSourceRTPSession(InetMcastAddress(senderIP), senderPort);
		return false;
	}
	else
	{
		input = new FCNSourceRTPSession(InetHostAddress(senderIP), senderPort);
	}
