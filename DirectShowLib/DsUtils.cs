...
using FutureConcepts.Media.DirectShowLib.Dvd;
...
namespace FutureConcepts.Media.DirectShowLib
{
    ...
    static public class DsResults
    {
        ...
        //------- The error codes below are returned by WindowsMedia components ------------

        /// <summary>
        /// The writer has received samples whose presentation times differ by an amount greater than
        /// the maximum synchronization tolerance. You can set the synchronization tolerance by
        /// calling IWMWriterAdvanced::SetSyncTolerance.
        /// This error can occur when there is more than one stream, and the application sends samples
        /// for one stream at a faster rate than the other stream. At some point, the second stream will
        /// lag too far behind the first, and the writer will return this error code.
        /// </summary>
        public const int NS_E_LateOperation = unchecked((int)0xC00D002E);

        /// <summary>
        /// The sample is not valid. This can occur when an input script stream contains a script sample that is not valid.
        /// </summary>
        public const int NS_E_InvalidData = unchecked((int)0xC00D002F);

        /// <summary>
        /// The wrong number of preprocessing passes was used for the stream's output type.
        /// Typically, this error will be returned if the stream configuration requires a
        /// preprocessing pass and a sample is passed without first configuring preprocessing.
        /// You can check for this error to determine whether a stream requires a preprocessing pass.
        /// Preprocessing passes are required only for bit-rate-based VBR.
        /// </summary>
        public const int NS_E_InvalidNumPasses = unchecked((int)0xC00D0BD5);

        //------- The error codes below are returned from MangoCapture.dll --------------
        /// <summary>
        /// Generic failure from MangoCapture
        /// </summary>
        public const int MXE_E_Fail = unchecked((int)0x80041708);

        /// <summary>
        /// a method timed out -- same as HRESULT_FROM_WIN32(ERROR_NOT_READY)
        /// </summary>
        public const int MXE_E_Timeout = unchecked((int)0x80070015);

        /// <summary>
        /// Dead Mango failure from Mango Capture (Mango needs to be power cycled)
        /// </summary>
        public const int MXE_E_MangoDead = unchecked((int)0x8004DEAD);
    }
    ...
    static public class DsError
    {
        ...
        /// <summary>
        /// Determines if an HRESULT indicates a failure
        /// </summary>
        /// <param name="hr">HRESULT to check</param>
        /// <returns>true if failure, false if success/status</returns>
        ...
        /// <summary>
        /// Determines if an HRESULT indicates success
        /// </summary>
        /// <param name="hr">HRESULT to check</param>
    ...
    static public class DsUtils
    {
        ...
        /// <summary>
        /// Generates a FOURCC code
        /// </summary>
        /// <param name="fourChars">a four character string</param>
        /// <returns>DWORD that is the FOURCC</returns>
        public static int MakeFourCC(string fourChars)
        {
            if(string.IsNullOrEmpty(fourChars))
            {
                throw new ArgumentNullException("fourChars");
            }
            if (fourChars.Length != 4)
            {
                throw new ArgumentException("FOURCC means exactly four characters!", "fourChars");
            }

            byte[] chars = new byte[4];
            for (int i = 0; i < 4; i++)
            {
                chars[i] = System.Convert.ToByte(fourChars[i]);
            }

            return (int)((UInt32)(chars[0]) | (UInt32)(chars[1] << 8) | (UInt32)(chars[2] << 16) | (UInt32)(chars[3] << 24));
        }
    }
