...
bool H264Formatter::ReceiveParse(RTPPayload & rtpPayload, deque<DSPayload> * outDSPayloads)
{
	...
	if(naluType < NALUType::RFC3984Min)	//this is a normal NALU
	{
	...
	else if(naluType == NALUType::FU_A) //Fragmentation Unit - "A type"
	{
...
bool H264Formatter::ConcatPendingFragments(deque<DSPayload> * outDSPayloads)
{
	...
	{
		DSPayload p = DSPayload();
		for(int i = 0; i < rtpFragments->size(); i++)
		{
		...
		for(long m = 1, i = 0; (m < p.Length) && (i < rtpFragments->size()); i++)
		{
								   ...
								   deque<RTPPayload> * outRTPPayloads)
{
	...
	if(dsPayload.Length <= maxRTPPayload)
	{
