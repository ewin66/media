    ...
    __in_opt LPCWSTR pName)
{
    ...
    if (pName) {
        ...
        if (m_pName) {
            (void)StringCchCopy(m_pName, namelen + 1, pName);
        } else {
...
#endif
#pragma prefast(suppress:11, "Suppress warning related to Free() invalidating 'this' which is no applicable to CBaseAllocator::Free()")
        NotifySample();
