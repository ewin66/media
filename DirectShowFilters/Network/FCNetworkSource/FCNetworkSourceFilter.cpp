...
FCNetworkSourceFilter::FCNetworkSourceFilter(LPUNKNOWN pUnk, HRESULT *phr)
	: CSource(NAME("CFCNetworkSourceFilter"), pUnk, CLSID_FCNetworkSource, phr),
	  outputs(), payloadMap()
{
	payloadMap.AddMappingForH264(100); //HACK needs to be done by application
	...
	BSTR senderIP = SysAllocString(L"127.0.0.1");
	outputs[0]->ConnectToSender(senderIP, 5002, 100);
