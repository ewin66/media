...
/*static unsigned short stupid_buf[8000*10 + 1152];
static unsigned char stupid_char_buf[8000*10 + 1152];
static int stupid_ptr = 0;
*/
...
/*		if (stupid_ptr < 8000*10)
		{
			memcpy(stupid_buf+stupid_ptr, sOutBuf.pBuffer, sOutBuf.nSize);
			memcpy(stupid_char_buf+stupid_ptr, sInBuf.pBuffer, sInBuf.nSize);
			stupid_ptr += sInBuf.nSize;
			if (stupid_ptr >= 8000*10)
			{
				FILE* fp = fopen("c:/pcma_decoded.raw", "wb");
				if (!fp) while(1);
				fwrite(stupid_buf, 8000*10, sizeof(short), fp);
				fclose(fp);
				fp = fopen("c:/pcma_as_is.raw", "wb");
				if (!fp) while(1);
				fwrite(stupid_char_buf, 8000*10, sizeof(char), fp);
				fclose(fp);
			}
		}*/
